...
import { AgentOverlay } from '../components/live-estimator/AgentOverlay';
import { RoomProgress } from '../components/live-estimator/RoomProgress';
...
export function LiveEstimatorPage() {
  ...
  const handleMessage = useCallback((msg: ServerMessage) => {
    switch (msg.type) {
      ...
      case 'angle_request':
        if (msg.instruction) {
          setGuidanceText(msg.instruction);
          setIsStreaming(false);
        }
        break;
  ...
  const handleStart = useCallback((details: HomeDetails) => {
    ...
    // Send start message once connection is open — poll via ref to avoid stale closure
  ...
  return (
    <Layout hideChat>
      <Helmet>
        <title>Live Video Estimator — MaidLink</title>
        <meta name="description" content="Walk through your home on camera and get an instant AI cleaning estimate." />
      </Helmet>
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-2 mb-1">
            <Link to="/estimate" className="text-sm text-teal-600 hover:underline">Estimator</Link>
            <span className="text-gray-300">/</span>
            <span className="text-sm text-gray-500">Live Video</span>
          </div>
          <h1 className="text-2xl font-bold text-gray-900">Live Video Walkthrough</h1>
          <p className="text-gray-500 mt-1 text-sm">
            Turn on your camera, walk room by room, and our AI will guide you to a personalised estimate.
          </p>
        </div>
